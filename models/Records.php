...
class Records extends Model{
	...
	public function add($tipo,$data_er,$hora_er,$colab_ret = '', $tipo_vr = '', $placa_vr = '', $motorista_vr = '', $rg = '', $empresa_vr = '' , $veiculos_id = '',$chaves_id = '',$obs = '',$visitante = '',$iduser){
		if($tipo == '0'){
			$sql = "INSERT INTO registros (tipo,data_er,hora_er,colab_ret,chaves_id,id_user) VALUES(:tipo,:data_er,:hora_er,:colab_ret,:chaves_id,:id_user)";
			$stmt = $this->db->prepare($sql);
			$stmt->bindValue(":tipo", $tipo);
			...
			$sql = "INSERT INTO registros (tipo,data_er,hora_er,tipo_v,placa_v,motorista_v,rg,empresa_v,obs,id_user) VALUES(:tipo,:data_er,:hora_er,:tipo_v,:placa,:motorista,:rg,:empresa,:obs,:id_user)";
			$stmt = $this->db->prepare($sql);
			$stmt->bindValue(":tipo", $tipo);
			$stmt->bindValue(":data_er", $data_er);
			$stmt->bindValue(":hora_er", $hora_er);
			...
			if($visitante == '1'){
				$sql = "INSERT INTO registros(tipo,data_er,hora_er,placa_v,motorista_v,rg,empresa_v,obs,visitante,id_user)
				VALUES
				(:tipo,:data_er,:hora_er,:placa,:motorista,:rg,:empresa,:obs,:visitante,:id_user)";
				$stmt = $this->db->prepare($sql);				
				$stmt->bindValue(":tipo", $tipo);
				$stmt->bindValue(":data_er", $data_er);
				$stmt->bindValue(":hora_er", $hora_er);
				...
				$sql = "INSERT INTO registros (tipo,data_er,hora_er,veiculos_id,obs,visitante,id_user) VALUES(:tipo,:data_er,:hora_er,:veiculos_id,:obs,:visitante,:id_user)";
				$stmt = $this->db->prepare($sql);
				$stmt->bindValue(":tipo", $tipo);
				$stmt->bindValue(":data_er", $data_er);
				$stmt->bindValue(":hora_er", $hora_er);
	...
	public function getRecebColet($data1,$data2,$placa,$motorista,$empresa,$tipo){
        ...
        if(!empty($motorista)){
            $where[] = "registros.motorista_v LIKE '%".$motorista."%'";
        }
        if(!empty($empresa)){
            $where[] = "registros.empresa_v LIKE '%".$empresa."%'";
        }
        if(!empty($placa)){
            $where[] = "registros.placa_v LIKE '%".$placa."%'";
        }        
        $sql .= implode(' AND ', $where);
        $stmt = $this->db->prepare($sql);
        if(!empty($tipo)){
